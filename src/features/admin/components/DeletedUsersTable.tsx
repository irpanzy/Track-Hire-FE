...
interface DeletedUsersTableProps {
  ...
  onRestoreClick: (user: DeletedUser) => void
  onPermanentDeleteClick: (user: DeletedUser) => void
  ...
  if (isError) {
    return (
      <div className="glass flex h-64 flex-col items-center justify-center gap-3 rounded-xl text-zinc-500">
        ...
        <p className="text-sm">
          Failed to load deleted users. Please try again.
        </p>
      </div>
    )
  }
                ...
                >
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-3">
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/[0.08] bg-white/[0.04] opacity-60">
                        {user.avatarUrl ? (
                          <img
                            src={user.avatarUrl}
                            alt={user.name}
                  ...
                  <td className="px-4 py-3.5">
                    <span
                      className={cn(
                        'inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold opacity-60',
                        user.role === 'ADMIN'
                  ...
                  <td className="hidden px-4 py-3.5 sm:table-cell">
                    {user.isEmailVerified ? (
                      <CheckCircle className="h-4 w-4 text-emerald-400 opacity-60" />
                    ) : (
                  ...
                  <td className="px-4 py-3.5 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button
                        ...
                        title="Restore user"
                        className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium text-zinc-500 transition-colors hover:bg-emerald-500/10 hover:text-emerald-400"
                      >
