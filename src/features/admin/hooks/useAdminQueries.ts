import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'
import { toast } from 'sonner'
import { adminService } from '../services/adminService'
import type {
  UsersQueryParams,
  UsersResponse,
  DeletedUsersResponse,
} from '../types/adminType'
import type { AxiosError } from 'axios'

export function useUsers(params: UsersQueryParams) {
  return useQuery<UsersResponse, AxiosError<{ message?: string }>>({
    queryKey: ['admin', 'users', params],
    queryFn: () => adminService.getUsers(params),
    placeholderData: (prev) => prev,
  })
}

export function useDeleteUser(onSuccess?: () => void) {
  const queryClient = useQueryClient()

  return useMutation<
    { message: string },
    AxiosError<{ message?: string }>,
    string
  >({
    mutationFn: (id: string) => adminService.deleteUser(id),
    onSuccess: () => {
      toast.success('User deleted successfully')
      queryClient.invalidateQueries({ queryKey: ['admin', 'users'] })
      queryClient.invalidateQueries({ queryKey: ['admin', 'deleted-users'] })
      onSuccess?.()
    },
    onError: (error) => {
      toast.error(
        error.response?.data?.message ||
          'Failed to delete user. Please try again.'
      )
    },
  })
}

// Recycle Bin hooks
export function useDeletedUsers(params: UsersQueryParams) {
  return useQuery<DeletedUsersResponse, AxiosError<{ message?: string }>>({
    queryKey: ['admin', 'deleted-users', params],
    queryFn: () => adminService.getDeletedUsers(params),
    placeholderData: (prev) => prev,
  })
}

export function useRestoreUser(onSuccess?: () => void) {
  const queryClient = useQueryClient()

  return useMutation<
    { message: string; user: unknown },
    AxiosError<{ message?: string }>,
    string
  >({
    mutationFn: (id: string) => adminService.restoreUser(id),
    onSuccess: () => {
      toast.success('User restored successfully')
      queryClient.invalidateQueries({ queryKey: ['admin', 'users'] })
      queryClient.invalidateQueries({ queryKey: ['admin', 'deleted-users'] })
      onSuccess?.()
    },
    onError: (error) => {
      toast.error(
        error.response?.data?.message ||
          'Failed to restore user. Please try again.'
      )
    },
  })
}

export function usePermanentDeleteUser(onSuccess?: () => void) {
  const queryClient = useQueryClient()

  return useMutation<
    { message: string },
    AxiosError<{ message?: string }>,
    string
  >({
    mutationFn: (id: string) => adminService.permanentDeleteUser(id),
    onSuccess: () => {
      toast.success('User permanently deleted')
      queryClient.invalidateQueries({ queryKey: ['admin', 'deleted-users'] })
      onSuccess?.()
    },
    onError: (error) => {
      toast.error(
        error.response?.data?.message ||
          'Failed to permanently delete user. Please try again.'
      )
    },
  })
}
